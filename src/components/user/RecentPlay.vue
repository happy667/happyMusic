<template>
  <div class="recent-play-container" ref="container">
    <!-- 头部导航栏 -->
    <van-sticky>
    ...
    <!-- 标签页 -->
    <van-tabs title-active-color="#FD4979" color="#FD4979" animated swipeable swipe-threshold="6">
      <!-- 单曲页 -->
      <van-tab :title="songTitle">
        <song @songSize="handleSongSize" />
      </van-tab>
      <!-- 专辑 -->
      <van-tab :title="albumTitle">
        <album @albumSize="handleAlbumSize" />
      </van-tab>
      <!-- 歌单页 -->
      <van-tab :title="songSheetTitle">
        <songSheet @songSheetSize="handleSongSheetSize" />
      </van-tab>
      <!-- 视频页 -->
      <van-tab :title="videoTitle">
        <Video @videoSize="handleVideoSize" />
      </van-tab>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleSongSize(size) {
        this.songTitle = `单曲 ${size}`
      },
      handleAlbumSize(size) {
        this.albumTitle = `专辑 ${size}`
      },
      handleSongSheetSize(size) {
        this.songSheetTitle = `歌单 ${size}`
      },
      handleVideoSize(size) {
        this.videoTitle = `视频 ${size}`
      }
