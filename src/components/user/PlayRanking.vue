<template>
  <div class="play-ranking-container">
    ...
    <van-tabs title-active-color="#FD4979" color="#FD4979" animated v-model="index" @change="handleChange" swipeable>
      <van-tab title="最近一周">
        <scroll ref="songList_weekend_scroll">
          <song-list ref="songList_weekend_container" :list="weekendSongList" :loading="loading" noResult="暂无听歌记录" />
        </scroll>
      </van-tab>
      <van-tab title="所有时间">
        <scroll ref="songList_all_scroll">
          <song-list ref="songList_all_container" :list="allSongList" :loading="loading" noResult="暂无听歌记录" />
        </scroll>
      </van-tab>
    </van-tabs>
...
<style lang="stylus" scoped>
  ...
  .play-ranking-container>>>.van-tabs__content .van-tabs__track,
  .play-ranking-container>>>.van-tabs__content .van-tab__pane {
    position: absolute;
    top: 0;
    bottom: 0;
  }
