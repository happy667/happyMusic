...
<song-sheet-list @select="selectSongSheet" :list="recommendSongSheet" v-if="this.recommendSongSheet.length > 0 ">
    <template>
            <Title path='/SongSheetSquare' loadMore styleInfo="margin-bottom:0.2rem" title="推荐歌单"></Title>
          </template>
</song-sheet-list>

<!-- 新碟上线 -->
<album-swiper :list="recommendNewAlbum" v-if="this.recommendNewAlbum.length > 0 ">
    <template>
