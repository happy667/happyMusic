      ...
      <div class="ranking-list-wrapper">
        ...
        <template v-if="rankingList2.length > 0">
          <div class="list-container">
            ...
            <common-list :rankingList="rankingList2"> </common-list>
          </div>
        </template>
        <!-- 曲风榜 -->
        <template v-if="rankingList3.length > 0">
          <div class="list-container">
            ...
            <common-list :rankingList="rankingList3"> </common-list>
          </div>
        </template>
        <!-- 全球榜-->
        <template v-if="rankingList4.length > 0">
          <div class="list-container">
            ...
            <common-list :rankingList="rankingList4"> </common-list>
          </div>
        </template>
        <!-- 语种榜-->
        <template v-if="rankingList5.length > 0">
          <div class="list-container">
            ...
            <common-list :rankingList="rankingList5"> </common-list>
          </div>
        </template>
        <!-- 特色榜-->
        <template v-if="rankingList6.length > 0">
          <div class="list-container">
            ...
            <common-list :rankingList="rankingList6"> </common-list>
          </div>
        </template>
        <!-- loading -->
...
export default {
  computed: {
    ...
    loadMore () {
      return (
        this.rankingList1.length > 0 &&
        (this.rankingList2.length === 0 ||
          this.rankingList3.length === 0 ||
          this.rankingList4.length === 0 ||
          this.rankingList5.length === 0 ||
          this.rankingList6.length === 0)
      );
    },
  },
  data () {
  ...
  methods: {
    ...
    async getRankingListByType (list, type) {
      for (let id of this.rankingListIds[type]) {
        const { data: res } = await rankingApi.getRankingListById(id);
        if (res.code === ERR_OK) {
  ...
  mounted () {
    // 如果当前已是排行tab（index=1），直接加载数据
    if (this.homeCurrentIndex === 1) {
      this.loadData();
    }
  },
...
.ranking-container .list-container :deep(.official-list-container:last-child) {
  margin-bottom: 0;
}
