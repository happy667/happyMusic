<template>
  <div class="songs-list-item-containter" :class="disableCls?'disable':''">
    <!-- 索引 -->
    <div class="song-index" :class="top?'rank':''" v-if="showIndex">{{index}}</div>
    <!-- 歌曲图片 -->
    <div class="song-img" v-if="showImage">
    ...
    <article class="right-info">
      ...
      <div class="icon-container" v-if="song.st>=0">
        <div v-if="song.mv" class="icon video" @click.stop="selectItemVideo">
          <van-icon name="tv-o" />
        </div>

        <div v-if="showPlayCount" class="play-count icon">
          <div class="icon">
            <van-icon name="play-circle-o" />
          </div>
          <div class="count">{{song.playCount}}次</div>
        </div>
        <div v-else class="icon love" :class="song.isLike ? 'active' : ''" @click.stop="selectItemLove">
