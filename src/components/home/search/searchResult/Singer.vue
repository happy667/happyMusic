  ...
  <div class="search-singer-container">
    ...
    <template v-if="singer.singerList.length!==0">
      <van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="handlePullingUp">
        <singer-list :list="singer.singerList" imageSize="middle" @select="handleSelect"></singer-list>
      </van-list>
    </template>
    ...
    export default {
        ...
        methods: {
            ...
            async getSearchSinger() {
                ...
                const {
                    data: res
                } = await searchApi.getSearchResult(this.searchKeywords, SEARCH_TYPE.singer, offset, 15)
                if (res.code === ERR_OK) {
                    // 没有查询到数据
