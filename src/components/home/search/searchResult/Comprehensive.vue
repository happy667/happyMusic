...
<script>
import { SEARCH_TYPE, VIDEO_TYPE } from "@/assets/common/js/config.js";
  ...
  methods: {
    ...
    async getSearchComprehensive() {
      ...
      if (res.code === ERR_OK) {
        ...
        // 处理专辑结果
        result.album = await this.handleAlbum(res.result.albums);
        // 处理歌单结果
        result.songSheetawait = this.handleSongSheet(res.result.playlists);

        // 处理视频结果
        //result.video = this.handleVideo(res.result.video)
        // 处理排序结果
        console.log(res.result.order);
        this.order = await this.handleOrder(res.result.order);
    ...
    async handleAlbum(album) {
      ...
      await Promise.all(
        album.map(async (item) => {
          ...
          if (!album.picUrl) {
            //没有图片就根据专辑id查询专辑详情获取图片（接口未返回）
            const { data: res } = await singerApi.getAlbumInfo(album.id);
            if (res.code === ERR_OK) {
    ...
    handleVideo(video) {
      ...
      // 过滤一遍视频,只需要mv类型视频
      video.videos
        .filter((item) => item.type === VIDEO_TYPE.mv)
        .forEach((item) => {
          let video = new Video({
            id: item.vid,
