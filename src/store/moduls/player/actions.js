...
export default {
  ...
  // 切换播放模式
  changeMode ({ state, commit, getters }) {
    const mode = (state.playMode + 1) % 3
    commit('setPlayMode', mode)
    const list = mode === PLAY_MODE.random
      ? utils.randomList(state.sequenceList)
      : state.sequenceList
    const index = list.findIndex(item => item.id === getters.currentSong.id)
    commit('setCurrentPlayIndex', index)
    commit('setPlayList', list)
